...
func expandRecord(_ record: Record) -> Record {
    (
        springs: (1...5).map { _ in record.springs }.joined(separator: "?"),
        arrangement: (1...5).flatMap { _ in record.arrangement }
    )
}
...
while let record = readRecord() {
    result += arrangements(expandRecord(record))
}
print(result)
