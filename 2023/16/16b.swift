...
func energize( _ map: Map, start: State) -> Int {
...
let map = readMap()
let starts: [State] = (1..<map.count).map { x -> State in
    State(index: Index(x: x, y: 0), direction: .down)
} + (1..<map.count).map { x -> State in 
    State(index: Index(x: x, y: map.count-1), direction: .up) 
} + map.indices.map { y -> State in
    State(index: Index(x: 0, y: y), direction: .right)
} + map.indices.map { y -> State in
    State(index: Index(x: map[0].count-1, y: y), direction: .right)
}

let result = starts
    .map { energize(map, start: $0) }
    .max()!
print(result)
